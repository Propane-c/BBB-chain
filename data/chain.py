...
class Chain(object):
    ...
    def get_feasible_keyblocks(self):
        ...
        for kb in kbs:
            ...
            if self.background.get_enable_gas():
                if kb.keyfield.pre_iz_pulp is None and self.background.get_enable_gas():
    ...
    def get_acpmbs_after_kb_and_label_forks(self, keyblock:Block) -> list[Block]:
        ...
        while queue:
            ...
            for b in block.next:
                if not b.iskeyblock:
                    if b.minifield.bfthmd_state or self.background.get_enable_gas():
                        ftmd_mbs.append(b)
            if len(ftmd_mbs)==0:
                continue
            queue.extend(select_acp_mbs(ftmd_mbs))
        # 记录主链上所有的miniblock
        logger.info("Miner %s accept miniblocks: %s", self.miner_id, [b.name for b in acp_mbs])
