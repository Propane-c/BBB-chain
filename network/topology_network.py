...
class TopologyNetwork(Network):
    ...
    def __init__(self, miners: List[miner.Miner]):
        ...
        # 结果保存路径
        NET_RESULT_PATH = background.get_net_result_path()
    ...
    def gen_network_rand(self, ave_degree):
        ...
        bandwidths = {(u,v):(bw_adv if self.miners[u].is_adversary
                      and self.miners[v].is_adversary else bw_honest)
    ...
    def draw_and_save_network(self):
        ...
        node_colors = ["red" if self.miners[n].is_adversary else '#1f78b4'  
        ...
        RESULT_PATH = background.get_net_result_path()
    ...
    def gen_routing_gragh(self, blockname, routing_histroy_single_block, origin_miner):
        ...
        for n, d in trans_process_graph.nodes(data=True):
            if self.miners[n].is_adversary and n != origin_miner:
