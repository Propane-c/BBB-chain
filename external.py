...
def I(round, input_tape:list):  # noqa: E743
    # insert functionality
    # bitcoin backbone 和 blockchain,round,RECEIVE 均无关
    if round <= 0:
        print('round error')
    input_content = None
    if input_tape is not None or len(input_tape) != 0:
        for instruction in input_tape:
            if instruction[0] == "INSERT":
                input_content = instruction[1]
                break
...
def common_prefix(prefix1:Block, chain2:Chain):
    while prefix1:
        if chain2.search_chain_backward(prefix1):
            break
        else:
            prefix1 = prefix1.pre
    return prefix1
...
def chain_quality(blockchain:Chain):
    ...
    if not blockchain.head:
        ...
        xc = []
        while blocktmp:
            xc.append(blocktmp.isAdversary)
...
def printchain2txt(miner,chain_data_url='chain_data.txt'):
    ...
    CHAIN_DATA_PATH=context.get_chain_data_path()
    ...
    with open(CHAIN_DATA_PATH /chain_data_url,'w+') as f:
        ...
        while q:
            ...
            print("isAdversaryBlock:",block.isAdversary,'\n'
            ...
            "Miner:",block.blockhead.miner_id,'\n'
